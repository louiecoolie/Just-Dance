...
local function isVIPServer()
	...
	if not(game.PlaceId == 7175796352) then --- if not lobby place then
		return true
	end
	...
	if data then
		...
		if not currentServerData then
			...
			if data.GameEnded then
				...
				print("a game has ended, remove from the list")
			else
...
local function getServersForClientWithFilter(client, filter)
	...
	for i, v in pairs(activeServerTable) do
		...
		print(i,v, "server table and children")
	end
	...
	for serverId, serverData in pairs(activeServerTable) do
		...
		if not filter["SearchDevBranch"] then
			if isDevBranch then
				includable = false
				print("dont include dev branch")
			end
		else
			if not isDevBranch then
				includable = false
				print("only include dev branch")
			end
		end
		
		if isPrivate then
			print("found private server")
		...
		if includable and (serverData.Healthy or filter["IncludeSlowServers"]) then
			...
			print("map ok, playersOk, serverTypeOk", mapOk, playersOk, serverTypeOk, modeOk, serverData.Type)
...
function matchmakingService:Init(m)
	...
	if not (isVIPServer()) then
		...
		local passed, errorMsg = pcall(function()
			...
			--end
			local test = ServerReporter:SubscribeSafe(onServerUpdateReceived)
	...
	remotes.RequestMatchmaking.OnServerEvent:connect(function(client, request, data, filter, count)
		...
		if request == "MainMenu" then
			teleportService:TeleportAsync(7175796352, {client})
		end
	end)
	...
	remotes.RequestVIPSettings.OnServerInvoke = function(client, request, params)
		...
		print(isVIPServer(), "checking vip")
